...
public class Searching_Nama {
    public static void main(String[] args) {
        ...
        String nama_arr[] = {"Andri Hariadi","Dewi Lestari","Dewi Agustin","Reni Indrayati","Toni Sukmawan","Toni Gunawan"};
        String alamat[] = {"Bandung","Surabaya","Malang","Malang","Surabaya","Bandung"};

        int n = nama_arr.length;
        ArrayList <Integer> posisi = cariLinear(nama_arr, n, "Tiara");

        if(posisi.size() > 0){
            for (int i = 0; i < posisi.size(); i++) {
                int indeks = posisi.get(i);
                System.out.println((i + 1) + ". " + nim[indeks] + " - " + nama_arr[indeks] + " - " + alamat[indeks] + " - " + "Data nomor " + (indeks + 1));
            }
        }
        else{
